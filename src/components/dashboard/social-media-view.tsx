...
import { SocialPost, SocialNetworkLink, SocialContentType, SocialTone, SocialNetworkType } from '@/lib/types';
...
const NETWORKS: { id: SocialNetworkType; label: string; color: string; bg: string; icon: React.ReactNode; homeUrl: string }[] = [
  {
  ...
  },
  {
    ...
    color: 'text-gray-900', bg: 'bg-black',
  ...
  },
  {
    ...
    color: 'text-red-600', bg: 'bg-red-600',
  ...
  },
  {
    ...
    color: 'text-blue-700', bg: 'bg-blue-700',
...
export function SocialMediaView({ posts, networkLinks, userId }: SocialMediaViewProps) {
  ...
  // Editor
  const [activeTab, setActiveTab] = useState<SocialContentType>('Plantilla');
  ...
  const filtered = useMemo(
    () => posts.filter(p => p.contentType === activeTab).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)),
    [posts, activeTab]
  );
  ...
  // ── Editor helpers ────────────────────────────────────────────────────────
  const reset = () => { setTitle(''); setMainText(''); setHashtags(''); setCta(''); setEditingId(null); setStatus('Borrador'); setTargetNets(['Instagram']); setTone('profesional'); };
  ...
  const handleSave = () => {
    ...
    const data: SocialPost = {
      id, contentType: activeTab, title: title.trim(), mainText: mainText.trim(),
  ...
  const handleGenerate = async () => {
    ...
    try {
      const r = await generateSocialContent({
        ...
        contentType: activeTab === 'Plantilla' ? 'plantilla' : activeTab === 'Contenido' ? 'contenido' : 'post',
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 animate-in fade-in duration-500">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={v => { setActiveTab(v as SocialContentType); reset(); }}>
              ...
              {filtered.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-black uppercase tracking-wide text-muted-foreground px-1">
                    {CONTENT_LABELS[activeTab]} guardadas ({filtered.length})
                  </p>
                  {filtered.map(p => {
