...
export function TenantHome({ tenantEntry, onNavigate }: TenantHomeProps) {
  ...
  const { whatsappNumber: adminWhatsApp } = useAdminWhatsApp(tenantEntry.adminId);
      ...
      {adminWhatsApp && (
        <div className="flex items-center gap-3 p-4 rounded-xl border border-green-200 bg-green-50">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-green-700">Consultá cualquier duda directamente</p>
          </div>
          <a
            href={`https://wa.me/${adminWhatsApp.replace(/[\s\-\(\)\+]/g, '')}?text=${encodeURIComponent(`Hola, soy ${tenantEntry.tenantName}, inquilino de ${tenantEntry.propertyName}. Quería consultarle sobre...`)}`}
            target="_blank"
